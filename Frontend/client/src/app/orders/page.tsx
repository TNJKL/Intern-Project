...
const MOCK_ORDERS = [
  {
    ...
    status: "delivering", // pending, processing, delivering, completed, cancelled
    total: 103000,
    items: [
...
export default function OrdersPage() {
  ...
  const filteredOrders = MOCK_ORDERS.filter(order => {
    if (activeTab === "active") {
      return ["pending", "processing", "delivering"].includes(order.status);
    }
          ...
          <AnimatePresence mode="wait">
            {filteredOrders.length === 0 ? (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -10 }}
                className="bg-white rounded-3xl p-12 text-center shadow-sm flex flex-col items-center"
              >
              ...
              filteredOrders.map(order => {
                ...
                return (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="bg-white rounded-3xl p-6 shadow-sm border border-transparent hover:border-gray-100 transition-all overflow-hidden"
                  >
                    ...
                    {/* Timeline Tracker (Chỉ hiện cho đơn đang giao) */}
                    ...
                    <div className="flex justify-end gap-3">
                      ...
                      <button 
                        ...
                        className="px-6 py-3 bg-gray-800 text-white rounded-xl font-bold hover:bg-black transition-colors text-sm uppercase tracking-wider flex items-center gap-2"
                      >
        ...
        {selectedOrder && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-white rounded-[32px] overflow-hidden shadow-2xl z-10 max-h-[90vh] flex flex-col"
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between p-6 border-b border-gray-100">
                ...
                <button 
                  onClick={() => setSelectedOrder(null)}
                  className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto space-y-6 flex-1 custom-scrollbar">
                ...
                <div className={`p-4 rounded-2xl flex items-center gap-4 ${getStatusDisplay(selectedOrder.status).bg}`}>
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center bg-white ${getStatusDisplay(selectedOrder.status).color}`}>
                    {(() => {
                      const Icon = getStatusDisplay(selectedOrder.status).icon;
                      return <Icon className="w-6 h-6" />;
                    })()}
                  </div>
                  <div>
                    <p className={`font-black uppercase text-sm ${getStatusDisplay(selectedOrder.status).color}`}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 rounded-2xl p-4 space-y-3">
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-800">Nguyễn Văn A - 0987654321</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 font-bold uppercase mb-1">Địa chỉ</p>
                      <p className="text-sm font-medium text-gray-800 leading-relaxed">Tòa nhà FPT, Khu CNC Hòa Lạc, Thạch Thất, Hà Nội</p>
                    </div>
                  </div>
                </div>
                ...
                {/* Order Summary */}
                <div className="space-y-4">
                  <h4 className="font-bold text-gray-800 uppercase tracking-wider text-sm">Sản phẩm đã chọn</h4>
                  <div className="space-y-3">
                    {selectedOrder.items.map((item: any) => (
                      <div key={item.id} className="flex items-center gap-3">
                        ...
                        <div className="text-right">
                          <p className="text-sm font-bold text-gray-800">{(item.price * item.quantity).toLocaleString('vi-VN')}đ</p>
                  ...
                  <div className="pt-4 border-t border-gray-100 space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-bold text-gray-800">{selectedOrder.total.toLocaleString('vi-VN')}đ</span>
                    </div>
                    <div className="flex justify-between text-sm">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-bold text-primary">-15.000đ</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
                ...
                {selectedOrder.status === 'completed' && (
                  <button className="px-6 py-3 bg-gray-800 text-white rounded-xl font-bold hover:bg-black transition-colors uppercase tracking-widest text-sm">
                    Mua lại
                  </button>
                )}
                {selectedOrder.status !== 'completed' && selectedOrder.status !== 'cancelled' && (
                  <button className="px-6 py-3 bg-red-50 text-red-500 rounded-xl font-bold hover:bg-red-100 transition-colors uppercase tracking-widest text-sm">
                    Hủy đơn
                  </button>
                )}
              </div>

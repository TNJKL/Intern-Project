...
const MOCK_CART_ITEMS = [
  { id: "1", name: "Phin Sữa Đá", price: 29000, quantity: 2, size: "M" },
  { id: "2", name: "Trà Sen Vàng", price: 45000, quantity: 1, size: "L" }
];
      ...
      <div className="min-h-screen bg-[#fcf9f2] flex flex-col items-center justify-center p-6 pb-32">
        ...
        >
          ...
          <p className="text-gray-500 mb-8 leading-relaxed">
            Cảm ơn bạn đã đặt hàng. Email xác nhận đã được gửi. Chúng tôi sẽ sớm giao những ly cà phê tuyệt hảo đến tay bạn.
          </p>
        ...
        <form onSubmit={handleCheckout} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-transparent hover:border-gray-100 transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <input required type="text" placeholder="Nhập tên người nhận" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-gray-700 block">Số điện thoại</label>
                  <input required type="tel" placeholder="Nhập số điện thoại" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all" />
                </div>
                <div className="space-y-2 md:col-span-2">
                  <label className="text-sm font-bold text-gray-700 block">Địa chỉ nhận hàng</label>
                  <input required type="text" placeholder="Số nhà, tên đường, phường/xã, quận/huyện..." className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all" />
                </div>
                <div className="space-y-2 md:col-span-2">
                  <label className="text-sm font-bold text-gray-700 block">Ghi chú (Tùy chọn)</label>
                  <textarea placeholder="Ghi chú thêm cho quán (ví dụ: ít đá, không đường...)" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 outline-none focus:bg-white focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all resize-none h-24"></textarea>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-3xl p-6 shadow-sm sticky top-24">
              ...
              <div className="space-y-4 mb-6 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                {MOCK_CART_ITEMS.map((item) => (
                  <div key={item.id} className="flex justify-between items-start">
                    ...
                    <span className="font-bold text-sm text-gray-800 shrink-0">{(item.price * item.quantity).toLocaleString('vi-VN')}đ</span>
                  </div>
                ))}
              </div>
              ...
              <button type="submit" className="w-full bg-primary text-white py-4 rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-coffee-dark shadow-lg shadow-primary/30 transition-all active:scale-[0.98]">
                Đặt Hàng Ngay
              </button>
            </div>
          </div>

...
export const CategoryModal: React.FC<CategoryModalProps> = ({
  ...
  return (
    <Modal
      ...
      centered
      // 🎯 ĐÃ CHỈNH SỬA 1: Giữ nguyên width={800} cho PC/Laptop luôn rộng rãi và giữ form chuẩn
      width={800}
      // 🎯 ĐÃ CHỈNH SỬA 2: Thêm class chặn chiều rộng tối đa dựa trên chiều rộng màn hình thực tế (viewport) để tự co khi sang điện thoại
      className="w-full max-w-[calc(100vw-24px)] sm:max-w-[600px] md:max-w-[720px] lg:max-w-[800px] rounded-3xl overflow-hidden"
      footer={(
        // Tối ưu hóa khu vực footer co giãn linh hoạt
        <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-4 px-6 md:px-10 pb-6 md:pb-10 pt-4 border-t border-gray-50 bg-white">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
            <Button
              onClick={onClose}
              className="rounded-xl font-bold h-11 border-gray-200 text-gray-500 flex-1 sm:flex-initial sm:px-8"
            >
              Đóng
            </Button>
            <Button
              ...
              onClick={handleSave}
              loading={isLoading}
              className="bg-[#d37533] rounded-xl font-bold h-11 flex-1 sm:flex-initial sm:px-10 border-none shadow-lg shadow-orange-100"
            >
    ...
    >
      ...
      <Form
        form={form}
        layout="vertical"
        className="px-6 md:px-10 pb-6 md:pb-10 pt-4 max-h-[65vh] md:max-h-[70vh] overflow-y-auto scrollbar-hide bg-white"
      >
        <div className="flex flex-col gap-6 w-full">
          {/* Nhóm 1: Thông tin cơ bản */}
          <div className="space-y-1">
            {/* 🎯 ĐÃ CHỈNH SỬA 3: Trên Mobile (xs) chiếm trọn 24 dòng dọc, từ Máy tính bảng (sm) trở lên chia 12/12 hàng ngang */}
            <Row gutter={[24, 0]}>
              <Col xs={24} sm={12}>
                <Form.Item
                  name="name"
              ...
              <Col xs={24} sm={12}>
                <Form.Item
                  ...
                  extra={<span className="text-[10px] text-gray-400 italic block mt-0.5">Đường dẫn không dấu, ví dụ: ca-phe-truyen-thong</span>}
                >
            ...
            >
              <Input.TextArea rows={4} placeholder="Mô tả ngắn về nhóm sản phẩm này..." className="rounded-xl border-gray-200 focus:border-[#d37533] text-sm p-4 resize-none" />
            </Form.Item>
          </div>

          {/* Nhóm 2: Hình ảnh */}
          <div className="pt-6 border-t border-gray-100">
            {/* 🎯 ĐÃ CHỈNH SỬA 4: Sử dụng sm:flex-row để trên laptop ảnh nằm cạnh input, trên mobile ảnh tự nhảy lên trên đầu input */}
            <div className="flex flex-col sm:flex-row gap-6 md:gap-8 items-center sm:items-start">
                ...
                <div className="w-full h-full rounded-2xl bg-white border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden relative group hover:border-[#d37533]/30 transition-all shadow-sm">
                  {imageUrl ? (
                    ...
                    <div className="text-center opacity-20">
                      <UploadOutlined className="text-xl md:text-2xl" />
                    </div>
                  )}
                  {isUploading && (
              ...
              <div className="flex-1 w-full space-y-3.5">
                <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest block text-center sm:text-left">Hình ảnh đại diện</span>
                <Upload {...uploadProps} className="w-full block">
                  <Button block className="rounded-xl h-11 font-bold border-gray-200 hover:border-[#d37533] hover:text-[#d37533] flex items-center justify-center gap-2 text-sm">
